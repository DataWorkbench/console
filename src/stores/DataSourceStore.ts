...
type OP = '' | 'create' | 'update' | 'enable' | 'disable' | 'delete' | 'view' | 'ping'
...
class DataSourceStore {
  ...
  itemLoadingHistories: Record<string, Map<string, Record<string, Record<string, any>[]>>> = {}
  ...
  itemHistories: Record<string, any> = {}

  updateItemHistory = (id: string, status: Record<string, any>) => {
    this.itemHistories = {
      ...this.itemHistories,
      [id]: status
    }
  ...
  addItemHistories = (sourceId: string, loadingItem: Record<'uuid' & string, any>) => {
    if (this.itemLoadingHistories[sourceId]) {
      this.itemLoadingHistories[sourceId].set(loadingItem.uuid, loadingItem)
    } else {
      this.itemLoadingHistories[sourceId] = new Map([[loadingItem.uuid, loadingItem]])
    }
  }
  ...
  removeItemHistories = (sourceId: string, loadingItem: Record<'uuid' & string, any>) => {
