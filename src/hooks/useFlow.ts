import { useMutation, useQuery, UseQueryOptions, useQueryClient } from 'react-query'
...
import { ConvertSyncJobModeRequestType, GenerateJobJsonRequestType } from '../types/request'
...
export const useFetchJob = () => {
  ...
  return useCallback(
    (tp: 'sync' | 'stream' = 'stream', filter = {}, options = {}) => {
      ...
      return queryClient.fetchQuery(['job', params], async () => listJobs(params), {
        // retry: 3,
        ...options
      })
    },
...
export const useMutationStreamJob = () => {
  const { regionId, spaceId } = useParams<IRouteParams>()
  return useMutation(
    async ({ op, jobMode, ...rest }: IWorkFlowParams & { jobMode: 'RT' | 'DI' }) => {
...
export const useQueryJobSchedule = (origin = '', options?: UseQueryOptions) => {
  ...
  const getJobSchedule = /^syj-/.test(curJob!.id) ? getSyncJobSchedule : getStreamJobSchedule
  streamJobScheduleKey = ['jobSchedule', params]
  return useQuery(streamJobScheduleKey, async () => getJobSchedule(origin, params), options)
}

export const useQueryStreamJobSchedule = (origin = '', options?: UseQueryOptions) => {
  const {
    workFlowStore: { curJob, curVersion }
  } = useStore()
  const { regionId, spaceId } = useParams<IRouteParams>()
  ...
  streamJobScheduleKey = ['jobSchedule', params]
  return useQuery(streamJobScheduleKey, async () => getStreamJobSchedule(origin, params), options)
}

export const useQuerySyncJobSchedule = (origin = '', options?: UseQueryOptions) => {
  const {
    workFlowStore: { curJob, curVersion }
  } = useStore()
  const { regionId, spaceId } = useParams<IRouteParams>()
  ...
  streamJobScheduleKey = ['jobSchedule', params]
  return useQuery(streamJobScheduleKey, async () => getSyncJobSchedule(origin, params), options)
}
...
export const useQueryStreamJobArgs = (options?: UseQueryOptions) => {
  ...
  return useQuery(
    key,
    async () => getStreamJobArgs({ ...params, regionId, spaceId, jobId: curJob?.id }),
    options as any
  )
}
...
export const useMutationPingSyncJobConnection = () => {
  ...
  const { regionId, spaceId } = useParams<IRouteParams>()
  return useMutation(async (params: { clusterId: string; sourceId: string; targetId: string }) =>
    pingSyncJobConnection({
...
export const useQueryStreamJobCode = (options?: UseQueryOptions) => {
  ...
  return useQuery(
    streamJobCodeKey,
    async () => getStreamJobCode({ ...params, regionId, spaceId, jobId: curJob?.id }),
    options
  )
}
...
export const useMutationReleaseStreamJob = () => {
  ...
  const { regionId, spaceId } = useParams<IRouteParams>()
  return useMutation(async (params: Record<string, string | boolean | number>) =>
