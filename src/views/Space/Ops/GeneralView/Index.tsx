...
import { Release, TextTitle, Example, Pie, StatusCard, Hexagon } from './components'
...
const Index = () => {
  ...
  const [devReleaseData, setDevReleaseData] = useState<{ value: number; name: string }[]>([])
  const [gatherReleaseData, setGatherReleaseData] = useState<{ value: number; name: string }[]>([])
  ...
  const [clusterData, setClusterData] = useState<{ value: number; name: string }[]>([])
  ...
  return (
    <FlexBox orient="column" tw="p-5 h-full">
      <FlexBox orient="column" tw="pt-6 pb-6 pl-5 pr-5 gap-5 bg-neut-16 rounded-[4px]">
      ...
      <FlexBox orient="column" tw="pt-6 pb-6 pl-5 pr-5 gap-5 bg-neut-16 rounded-[4px] h-[172px]">
        ...
        <FlexBox tw="items-center h-[80px]">
          <FlexBox tw="w-[50%] h-[80px] justify-between items-center">
            ...
            <Pie id="cluster" title="计算集群总数" amount={clusterTotal} pieData={clusterData} />
          </FlexBox>
