      ...
      <div tw="pt-6 px-3 pb-6">
        ...
        <Divider tw="my-3 border-line-dark text-white">
          <Center tw="cursor-pointer" onClick={() => setVisible(!visible)}>
            <Icon name={visible ? 'chevron-down' : 'chevron-up'} type="light" size={16} tw="mr-2" />
            高级配置
          </Center>
        </Divider>
          ...
          <Grid>
            ...
            <div>
              {target?.tableConfig?.preSql
                ? target?.tableConfig?.preSql.map((i: string) => <div key={i}>{i}</div>)
                : '无'}
            </div>
            <div>写入后SQL语句组</div>
            <div>
              {target?.tableConfig?.postSql
                ? target?.tableConfig?.postSql.map((i: string) => <div key={i}>{i}</div>)
                : '无'}
            </div>
          </Grid>
        </CollapsePanel>
