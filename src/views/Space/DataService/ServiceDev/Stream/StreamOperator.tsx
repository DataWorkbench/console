...
const StreamOperator = ({ className }: { className?: string }) => {
  ...
  const [{ canDrop, isOver }, drop] = useDrop(
    () => ({
      ...
      drop: (item, monitor) => {
        ...
        setElems([...elems, { ...item, x: x > 0 ? x : 0, y: y > 0 ? y : 0, id: nanoid() }])
      },
      collect: (monitor) => ({
  ...
  return (
    <div className={className} tw="flex flex-col text-white relative">
      ...
      <div ref={drop} css={[tw`flex-1`, isActive && tw`bg-neut-13 bg-opacity-30`]}>
          ...
          {elems.map((elem) => (
            <div
              ...
              css={[
                ...
                elem.type === 'table' ? tw`border-l-[#229CE9]` : tw`border-l-[#934BC5]`
              ]}
            >
              <div tw="w-6">
                <span
