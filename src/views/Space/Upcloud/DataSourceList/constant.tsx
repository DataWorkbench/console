...
export const CONNECTION_STATUS: Record<'LOADING' | 'FAIL' | 'SUCCESS' | 'UNDO', -1 | 0 | 1 | 2> = {
  LOADING: -1,
  UNDO: 0,
  SUCCESS: 1,
  FAIL: 2
...
export const hostReg = /^([0-9a-zA-Z_.-]+(:\d{1,5})?,)*([0-9a-zA-Z_.-]+(:\d{1,5})?)?$/
...
export const hiveAnonymousFilters = new Set(['defaultFS', '__dbUrl', 'hiveAuth', 'config'])

export const esPwdFilters = new Set(['__dbUrl', 'esAuth', 'version', 'user', 'password'])
    ...
    switch (type) {
      ...
      case 'mongo_db':
        return `mongodb://${urlObj.hosts
          .map(({ host, port }: { host: string; port: number }) => `${host}:${port}`)
      ...
      case 'ftp':
        return `${lowerCase(get(ftpProtocol, `${urlObj?.protocol}.label`))}://${urlObj?.host}:${
          urlObj?.port
        }`
      case 'hdfs':
